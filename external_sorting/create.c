...
void create_bin_file_double(char* file_name, int num) {
	double r;
	FILE* file_out = fopen(file_name, "wb");
	for(int i = 0; i < num;i++){
		r = rand() % 100;
		fwrite(&r, sizeof(double), 1, file_out);
	}
	fclose(file_out);
}

void reader_bin_file_double(char* file_name){
	...
	double numbers;
	printf("\nReading file . . . %s \n", file_name);
	while(fread(&numbers, sizeof(double), 1, file_in)){
		printf("%lf\n", numbers);
	}
	fclose(file_in);
}
...
void convert_txt_file_to_bin_file(char* file_name_txt){
	...
	while(fscanf(file_txt, "%lf", &numbers) != EOF){
		fwrite(&numbers, sizeof(double), 1, bin_file);
	}
	fclose(bin_file);
	fclose(file_txt);

	printf("\nReading converted file: \n");
	reader_bin_file_double(bin_file_name);
...
int main() {
	...
	if (op == 1) {
		...
		scanf("%d",&num);
		create_bin_file_double(s, num);
	} else if (op == 2) {
		printf("Enter the file name: ");
		scanf("%s", s);
		reader_bin_file_double(s);
	} else if (op == 3) {
		printf("Enter the file name: ");
		scanf("%s", s);
